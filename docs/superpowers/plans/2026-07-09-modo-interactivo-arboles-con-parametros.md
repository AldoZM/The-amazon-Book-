...
- Produces:
  ...
  - `VIS.arbol.campo(): field` — el descriptor del campo de texto del árbol. Idéntico en los tres editores, así que vive una vez.
  - `VIS.arbol.parseCon(state, numericos: string[]): {ok:true, input} | {ok:false, field:"tree", error}` — parsea el árbol con `MAX_NODOS` y convierte a número los campos nombrados en `numericos`. El límite de nodos deja de estar copiado en cada problema.
...
**Interfaces:**
- Consumes: `VIS.arbol.campo()`, `VIS.arbol.parseCon(state, numericos)`, `VIS.arbol.opcionesDeNodos(texto)`, `VIS.arbol.opcionesDeK(texto)`, `VIS.preview.tree(arr, label, resaltados)` (Task 1); `field.type: "select"` con `options(state)` (Task 2).
- Produces: `PROBLEMS[{236,1644,863}].editor` con `kind: "text"` y tres campos cada uno.
- **Ningún problema llama a `VIS.parse.treeArray` ni escribe el `15` a mano.** El límite de nodos vive en `VIS.arbol.parseCon`.
...
**Ojo con el orden de carga.** `VIS.arbol.campo()` se invoca **al cargar el módulo**
del problema (está en un literal de objeto, no dentro de una función), así que
`js/editors.js` tiene que estar cargado antes. Lo garantizan el `<script>` de
`problem.html`, la lista de `tools/validate-code.js` y la de `tools/test-editors.js`
— las tres ya lo cargan. Es el mismo motivo por el que `VIS.treeEditor` funciona.
...
- **`const` no se iza.** `MAX_NODOS` se usa dentro de `VIS.arbol.opcionesDeNodos` y de `VIS.arbol.parseCon`, así que su declaración tiene que aparecer **antes** en el archivo. Por eso la Task 1 lo mueve en un paso propio. Si lo olvidas, el fallo es un `ReferenceError` en tiempo de ejecución, no un error de sintaxis: no lo cazará `node --check`.
- **El límite de nodos vive en un solo sitio.** Ningún problema escribe el `15`: llaman a `VIS.arbol.parseCon`. Si ves un `VIS.parse.treeArray(state.tree, 15)` dentro de un `js/problems/*.js`, está mal.
- **`VIS.arbol.campo()` se invoca al cargar el módulo** del problema (está en un literal de objeto, no dentro de una función). `js/editors.js` debe cargarse antes: `problem.html`, `tools/validate-code.js` y `tools/test-editors.js` ya lo hacen.
